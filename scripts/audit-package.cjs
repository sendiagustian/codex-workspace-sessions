...
const allowed = /^(?:\[Content_Types\]\.xml|extension\.vsixmanifest|extension\/(?:package\.json|readme\.md|LICENSE\.txt|changelog\.md|media\/icons\/(?:sessions\.svg|usage\.svg|icon\.png)|media\/screenshots\/[123]\.png|media\/webview\/sessions\/sessions\.(?:html|css|js)|media\/webview\/usage\/(?:usage|usage-window|usage-bar|usage-bar-empty)\.html|media\/webview\/usage\/usage\.css|dist\/extension\.js|dist\/controller\/session-controller\.js|dist\/model\/(?:session|usage)\.js|dist\/services\/(?:account-usage|codex-integration|local-files|session-store|usage-reader)\.js|dist\/util\/json\.js|dist\/views\/(?:session-details-provider|session-view|usage-view|webview-assets)\.js|docs\/(?:INTEGRATION|PRIVACY|VALIDATION)\.md))$/;
...
async function inspectEntry(zip, entry) {
  ...
  if (entry.fileName === 'extension/media/icons/icon.png' || entry.fileName.startsWith('extension/media/screenshots/')) {
...
async function inspect() {
  ...
  for (const name of ['1.png', '2.png', '3.png']) assert.ok(files.includes(`extension/media/screenshots/${name}`));
